...
class LinsumLayerColumn(AbstractLayer):
    def __init__(
        self,
        num_features: int,
        num_sums_in: int,
        ...
        self.column_index = check_valid(column_index, int, 0)
    ...
    def _init_weights(self):
        ...
        num_sums_in = self.num_sums_in + self.column_index * self.num_sums_in
        ws = torch.randn(
    ...
    def forward(
        self, x: torch.Tensor, prev_columns_prod_outputs: List[torch.Tensor]
        ...
        # Current layer product output
        prod_output = (left + right).unsqueeze(3)  # N x D/2 x Sin x 1 x R

        # Concatenate current layer product output with all previous column product outputs of this layer index
        prod_output_all = torch.cat(prev_columns_prod_outputs + [prod_output], dim=2)

        # Get log weights
        log_weights = self._get_normalized_log_weights().unsqueeze(0)
        prob = torch.logsumexp(prod_output_all + log_weights, dim=2)  # N x D/2 x Sout x R

        return prob, prod_output
