...
class EinetColumn(nn.Module):
    ...
    def __init__(self, config: "EinetColumnConfig", column_index: int):
        """
        Create a RatSpn based on a configuration object.

        Args:
            config (EinetColumnConfig): RatSpn configuration object.
            column_index (int): Index of this column in the PSPN.
        """
        super().__init__()
        config.assert_valid()
        self.config = config
    ...
    def forward(
        self, x: torch.Tensor, prev_column_outputs: List[List[torch.Tensor]], marginalization_mask: torch.Tensor = None
        ...
        x, layer_outputs = self._forward_layers(x, prev_column_outputs)
    ...
    def _forward_layers(
        self, x, prev_column_outputs: List[List[torch.Tensor]]
    ) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        """
        ...
        # Collect layer outputs for possible next columns
        layer_outputs = []
        for i, layer in enumerate(self.layers):

            # Collect outputs of all previous column for the current layer
            prev_column_outputs_for_layer = []
            for column in prev_column_outputs:
                prev_column_outputs_for_layer.append(column[i])

            # Forward pass of the current layer with current column input and all outputs of the
            # previous columns at this layer
            x, prod_output = layer(x, prev_column_outputs_for_layer)

            layer_outputs.append(prod_output)
        return x, layer_outputs
    ...
    def _build(self):
        ...
        self.layers: Sequence[LinsumLayerColumn] = nn.ModuleList(reversed(einsum_layers))
...
class PSPN(nn.Module):
    ...
    def expand(self):
        # Disable gradients for all previous columns
...
class EinetColumnConfig:
    ...
    def assert_valid(self):
        ...
        self.seed = check_valid(self.seed, int)
